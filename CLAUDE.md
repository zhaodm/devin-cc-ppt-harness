...
- DE 开发阶段只能写入 deliverables/{REQ-ID}/output/pages/，绝对禁止写入根目录 output/final/（仅 archive 归档时使用）
- 角色间传递必须经 PM 中转
...
- 每步结束返回 PM，通过后才启动下一步
- 禁止跳过人工审批节点
- 每个流程（init/propose/apply/archive）结束时，PM 必须打印 workflow.md 中定义的「完成输出」模板，明确告知用户下一步操作
