...
| /ppt-apply | apply.sh | dev-test.md + post-verify.md | output/pages/*.html |
...
Step 1b: PM 生成任务编排计划，列出每页 DE→TE→人工检查 步骤，用户确认后开始

FOR 每个待开发页面（跳过已完成）:
    → PM 校验上一页「逐页人工检查记录」有 PASS（非首页时）
    → PM 写 handoff → DE 编码实现 → output/pages/{page}.html
    → PM 写 handoff → TE 审计验证 → temp-test-report.md
    → 失败？→ 新 handoff → DE 修复（最多5轮）
    → 逐页人工检查（轻量确认该页OK，无轮次限制）
    → PM 在 .state.md「逐页人工检查记录」追加 PASS
...
archive.sh 前置检查
  ...
  → 代码归档：
      cp output/pages/*.html → output/final/pages/
      cp output/shared/ → output/final/shared/
...
| archive.sh | 检查 SR3 PASS + output 非空 | /ppt-archive 前置 |
...
- 跳过已存在于 output/pages/ 的页面
- PM 恢复时读取 .state.md 确定恢复点，不依赖对话历史
- 恢复时通过「逐页人工检查记录」判断哪些页面已完成人工确认，未记录的页面必须重新走人工检查
...
| 2026-05-15 | 新增 4.4 Step 1b 任务编排确认 + 逐页人工检查状态追踪机制 |
